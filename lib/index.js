...
const toBool = (val, def) => val === 'true' || val === true ? true : val === 'false' || val === false ? false : def;
const isEV = (name) => envExists && Object.keys(process.env).some(k => k === name);
/********************************* GET & PROCESS ENVIRONMENT VALS *********************************/
const NODE_ENV = isEV('NODE_ENV') ? process.env.NODE_ENV.toLowerCase() : 'development';
const LOG_LEVEL = isEV('LOG_LEVEL') ? process.env.LOG_LEVEL.toLowerCase() : 'info';
const RELEASE_ENV = isEV('RELEASE_ENV')
...
const WAS_RUN_THRU_MOCHA = Object.keys(process.env).some(k => k === 'mocha' || k === 'LOADED_MOCHA_OPTS') &&
...
exports.isDebugMocha = WAS_RUN_THRU_MOCHA && exports.isDebug;
...
exports.isSillyMocha = WAS_RUN_THRU_MOCHA && exports.isSilly;
